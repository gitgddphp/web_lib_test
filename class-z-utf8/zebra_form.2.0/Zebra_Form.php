 ...
 *
 *  Requires PHP 4.2.0+ or PHP 5.0.2+
 *
 ...
 *  @version    2.0 (last revision: April 10, 2011)
...
class Zebra_Form
{
    ...
    function Zebra_Form($name, $method = 'POST', $action = '', $attributes = '')
    {
        ...
        $this->form_properties = array(
            ...
            'name'                      =>  $name,
            'other_suffix'              =>  '_other',
            'process_path'              =>  'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . $_SERVER['HTTP_HOST']. '/' . substr(trim(str_replace('\\', '/', substr(__FILE__, 0, -strlen(basename(__FILE__)))), '/'), strlen(str_replace('\\', '/', $_SERVER['DOCUMENT_ROOT']))) . '/process.php',
     ...
     *  </code>
     *
     *  When the form is rendered, some JavaScript code is also inserted along with a JavaScript object that provides
     *  a few public methods for interacting with the form from JavaScript. The object's name will be
     *  <i>zf_[form_name]</i> where <i>form_name</i> is the form's name <b>with any dashes turned into underscores!</b>
     *  <i>Therefore, if a form's name is "my-form", the JavaScript object's name will be "zf_my_form".</i>
     *
     *  From JavaScript, there are a few methods of this object that can be called:
     *
    ...
    function render($template = '', $return = false)
    {
        ...
        if ($this->form_properties['has_upload'] === true) {
            ...
            // if client-side validation is enabled
            if ($this->form_properties['clientside_validation'] != false) {

                // set the path to the process.php
                $this->form_properties['clientside_validation'] = array('process_path' => $this->form_properties['process_path']);
        ...
        foreach ($this->controls as $key => $control) {
            ...
            if ($this->form_properties['clientside_validation'] !== false && !empty($control->rules)) {
                    ...
                    switch ($rule) {
                        ...
                        case 'regexp':
                            ...
                            if (trim($properties[0]) != '') {
                                ...
                                $class .= '(' . preg_replace(array('/\[/', '/\]/', '/\,/'), array('lsqb;', 'rsqb;', 'comma;'), $properties[0]) . ')';
            ...
            if (isset($attributes['type']) && $attributes['type'] == 'text' && preg_match('/\bdate\b/i', $attributes['class'])) {
                ...
                $datepicker_javascript .= 'calendar_' . $attributes['id'] . ' = new Calendar({' . $attributes['id'] . ':\'' . $control->attributes['format'] . '\'}';
                ...
                foreach ($control->attributes as $attribute => $value) {
                    ...
                    // if the attribute is not "format" (as we added it above) and attribute is an attribute intended for
                    // the javascript object and is not null
                    if ($attribute != 'format' && in_array($attribute, $control->javascript_attributes) && $control->attributes[$attribute] != null) {
        ...
        // add a "timer"
        $this->add('hidden', $this->form_properties['timer'], $this->_encrypt(mktime() + 1234567890));
        ...
        if ($template == '' || $template == '*horizontal' || $template == '*vertical') {
            ...
            if ($template == '*horizontal') {
                ...
                foreach ($blocks as $controls) {
                    ...
                    $label = array_shift(array_values($controls));
        ...
        if (is_array($this->form_properties['clientside_validation'])) {
            ...
            foreach ($this->form_properties['clientside_validation'] as $key => $value) {
                ...
                // save property
                $javascript_object_properties .= ($javascript_object_properties != '' ? ',' : '') . $key . ':' . ($value === true ? 'true' : ($value === false ? 'false' : '\'' . $value . '\''));
        ...
        // append the javascript object to the end of the output
        $output .= '<script type="text/javascript">' . (isset($datepicker_javascript) ? $datepicker_javascript : '') .  'var zf_' . preg_replace('/\-/', '_', $this->form_properties['name']) . '=new Zebra_Form("' . $this->form_properties['name'] . '"' . ($javascript_object_properties != '' ? ',{' . $javascript_object_properties . '}' : '') . ')</script>';
    ...
    function validate()
    {
        ...
        if (
            ...
            if (
                ...
                mktime() - $this->_encrypt($method[$this->form_properties['timer']], true) + 1234567890 > 0
                    ...
                    switch ($rule_name) {
                        ...
                        case 'date':
                            ...
                            if (
                                ...
                                $format = preg_replace('/\s/', '', $control->attributes['format']);
                                    ...
                                    if (preg_match($format, preg_replace('/\s/', '', $attribute['value']), $segments)) {
                                        ...
                                        if ($valid) {
                                            ...
                                            if ($attribute['value'] == date($control->attributes['format'], strtotime($english_months[$original_month] . ' ' . $original_day . ', ' . $original_year))) {
                                                ...
                                                // make sure we return a UNIX timestamp for date controls
                                                $control->attributes['timestamp'] = strtotime($english_months[$original_month] . ' ' . $original_day . ', ' . $original_year);
                        ...
                        case 'datecompare':
                            ...
                            if (
                                ...
                                switch ($rule_attributes[1]) {
                                    case '>':
                                        $valid = ($control->attributes['timestamp'] > $this->controls[$rule_attributes[0]]->attributes['timestamp']);
                                        break;
                                    case '>=':
                                        $valid = ($control->attributes['timestamp'] >= $this->controls[$rule_attributes[0]]->attributes['timestamp']);
                                        break;

    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
			...
			foreach ($coupon as $key => $val)
			{
				// 送券            
                // 送券
                $coupon_data = array('cid'=>$val['id'],'type'=>$val['type'],'uid'=>$user['user_id'],'send_time'=>time(),'store_id'=>$val['store_id']);
                $coupon_data['use_end_time'] = $val['use_end_time'];
                switch($val['use_type']){
                    case 0 : $coupon_data['use_end_time'] = $val['use_end_time']; break;//延后一个月
                    case 1 : $coupon_data['use_end_time'] = time() + 3600*24*30*1; break;//延后一个月
                    case 2 : $coupon_data['use_end_time'] = time() + 3600*24*30*2; break;//延后二个月
                }
...
public function get_comment($user_id,$status=2,$p){
    ...
    //获取分享来源的信息
    public static function getSafyInfo($user_id=0){
        $data = [];
        if($user_id){
            $data = M('users')
            ->field('user_id,head_pic,nick_name')
            ->find($user_id);
            $data['head_pic'] = SITE_URL.$data['head_pic'];
        }
        return $data;
    }
}